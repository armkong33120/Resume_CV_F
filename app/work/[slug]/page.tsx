...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  return (
    <>
      {/* Hero Cover */}
      <div className="relative w-full h-[50vh] xs:h-[60vh] sm:h-[70vh] bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center safe-top">
        <div className="absolute inset-0 flex items-center justify-center text-foreground/20 text-lg xs:text-2xl sm:text-4xl font-semibold px-4">
          {project.title}
        </div>
      </div>
        ...
        <div className="mb-8 xs:mb-12 sm:mb-16 pb-6 xs:pb-8 sm:pb-12 border-b border-border">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 xs:gap-6 sm:gap-8 mb-4 xs:mb-6">
            <div className="flex-1">
              <h1 className="text-3xl xs:text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tighter text-foreground mb-3 xs:mb-4">
                {project.title}
              </h1>
          ...
          {project.highlights.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 xs:gap-4 sm:gap-6">
              {project.highlights.map((highlight, index) => (
                ...
                >
                  <p className="text-xs xs:text-sm text-foreground/80">{highlight}</p>
                </div>
              ))}
            </div>
          )}
